...
export async function downloadImage(imageUrl) {
  try {
    ...
    const outputPath = path.join(currentDir, `downloaded_image${fileExtension}`);
    ...
    if (response.status === 200) {
      ...
      return outputPath; // Return the path to the downloaded image
    } else {
      console.error(`Failed to download image. HTTP status code: ${response.status}`);
